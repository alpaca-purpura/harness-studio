...
3. Código existente: `web/src/pages/shell/ui/global-view.tsx` (la vista a estrenar) ·
   ...
   (patrón sentinels+servicio) · `scripts/bundle.sh` · `arch/contracts/api/openapi.yaml`
   · `arch/fitness/.go-arch-lint.yml` (añadir componente `selfupdate`)
4. Arquitectura que RIGE: `superficie-local-confinada` (endpoint bajo withAuth, CERO
   ...
   `arch/conventions/` (estilo Go/TS, commits `feat(HS-11): …`)
