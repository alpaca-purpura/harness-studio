...
**As-code:** filas en `arch/boundaries/contrato-de-caja-es-fitness-function.md` con
`enforced_by:` la función Go (sin-huerfanos ya existe deferred → apuntar al enforcer
real; 3 filas nuevas). Actualizar aritmética en `arch/INDEX.md` (97→100) y donde el
...
(`parser.go:105`, colisión knowledge/hooks ↔ conventions/hooks) — las filas nuevas van
en el boundary, no crear archivo nuevo con nombre colisionante.

## Fase 2 — identidad del art

**Schema:** `arch/contracts/schema/box.contract.schema.json` — en `$defs` de entrega
