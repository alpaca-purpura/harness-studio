...
### 1.1 · Boundaries FE (`arch/boundaries/fe-*.md`)
...
### 1.2 · Modelo C4 (`arch/model/`)

Existen `container.d2` + `system-context.mmd`. **No hay `INDEX.md`** en `model/` (el índice C4 vive en
`arch/INDEX.md`). El Mapa como container FE no está dibujado explícito → deuda menor (añadir el nodo
«Map surface» al `container.d2`, §9).

### 1.3 · Contratos (`arch/contracts/`)
...
### 1.6 · Convenciones (`arch/conventions/`)
...
| 2 | Activación por-nodo | `graph.l0.schema.json $defs.nodo` + `Box` + `knowledge/rules.md` | **añadir `alw`** (o derivar); mover honestidad a check evaluable. PROPUESTA |
