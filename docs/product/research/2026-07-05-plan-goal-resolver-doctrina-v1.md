...
2. **G2 · Knowledge invocable:** los checks del árbol `docs/architecture/knowledge/` se **ejecutan** (no solo se leen)
...
1. **Hexagonal / ports & adapters como columna.** El core del dominio (modelo de la doctrina: `Box`,
   ...
   modificabilidad.
2. **El conocimiento es DATO, no código.** El árbol `docs/architecture/knowledge/` (12 nodos · 138 checks) y `arch/`
...
7. **Cada puerto nuevo NACE como boundary.** `ConformancePort`, `RulesetPort`, `PermissionPort`,
   `ConductorPort` se declaran en `docs/architecture/boundaries/` con su check `enforced_by:` real (o `t.Skip`
...
**P0 · El estándar `docs/architecture/knowledge/` está mapeado pero NO cableado a ningún motor que lo invoque** (hallazgo
...
- **`RulesetPort`** (nuevo, salida): carga `docs/architecture/knowledge/` + `arch/` como un **ruleset de datos** (cada
...
- No romper boundaries `enforced` existentes (HS-06: superficie-local-confinada, sesion-viva-consistente;
  permisos-gui). `go test ./docs/architecture/fitness/...` sigue verde.
