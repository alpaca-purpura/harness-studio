...
> (`docs/architecture/knowledge/` — los 122 checks + L2) y no en re-derivar el mecanismo.
>
...
> Firmada junto con `docs/architecture/contracts/nomenclatura-arnes.md` v1 (D-a/D-b/D-c). Los conteos de
...
|---|---|---|---|
| **① Doctrina** (el know-how) | 11 elementos · 122 checks · plantillas · metodología · contrato de caja | `docs/architecture/knowledge/`+`arch/` en el repo → **compilada y `go:embed`** en el binario | el runner Go **Y** el meta-arnés |
...
REPO fábrica (fuente de verdad, versionada)
  docs/architecture/knowledge/elements/*.md   → L2 checklists = los 122 checks
...
Ambas derivan del **mismo** `docs/architecture/knowledge/elements/*.md`. El build las mantiene sincronizadas → un solo
...
- **Updates** = update del binario → re-materializa `kit/`+`doctrine.md`+`ruleset.json`+`knowhow/`. La
  doctrina evoluciona semanal (`docs/architecture/knowledge/CADENCE.md`) → cada release trae el know-how nuevo. Arneses ya
...
3. **`go:embed` del know-how compilado** + subcomando/paso `arnesia build-kit` (go:generate) que extrae
   `docs/architecture/knowledge/` → `ruleset.json` + `kit/` + `doctrine.md`.
...
| `doctrina-una-fuente-dos-targets` | un ruleset ejecutable y su guía generativa no deben divergir | `ruleset.json` (Go) y `kit/` (CC) se derivan ambos de `docs/architecture/knowledge/` en build; drift = check rojo | error |
