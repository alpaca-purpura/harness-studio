...
> paralelo (coherencia documental · árbol knowledge · docs/architecture/boundaries+schema · aplicabilidad ·
...
- **Dónde:**
  - `docs/architecture/contracts/schema/box.contract.schema.json:5` la propia descripción se ancla a "iteración 10
    de HS-03" (pre-doctrina); `:8` `"additionalProperties": false` (también en `gate` y en items de
    `entrega`).
  - `METODOLOGIA.md:137-138` afirma "Schema validado por `box.contract.schema.json`"; `docs/architecture/INDEX.md:16-17`
  ...
  - `docs/architecture/fitness/arch_test.go:191` `TestBoxContractValidatesAgainstSchema` está en `t.Skip("TODO fase 5")`.
...
- **Además:** el enum de METODOLOGIA usa labels legacy (`agente`≠`subagent`, `regla`≠`rule`), incluye
  ...
  → no cubre los 12 nodos de `docs/architecture/knowledge/`. `graph.l0.schema.json:38` `$comment` dice "TODO **HS-06**:
...
- **Dónde:** `docs/architecture/boundaries/orquestacion-determinista-entre-cajas.md:17` → `fitness/arch_test.go:TestConductorOwnsBoxRouting`;
  `docs/architecture/boundaries/permisos-derivan-del-rol.md:17` → `fitness/arch_test.go:TestPermissionSetParametrizedByRole`.
...
- **Dónde:** `docs/architecture/knowledge/INDEX.md:83-88` (autoconfesión) "hoy estos checks NO llevan `enforced_by`…";
  `docs/architecture/INDEX.md:22-25` idem; el único `arnesia conformance` en el repo es un **comentario**
...
- **Dónde:** `METODOLOGIA.md:301` "### 8.3 Document-as-cache … (**A7 de APM**)"; `docs/architecture/boundaries/orquestacion-determinista-entre-cajas.md:25,58` "(**A2 de Agentic BPM** + 12-Factor)". Fuente:
...
- **Dónde:** anatomía PROPIA firmada `VISION.md:61-87` (A2 = contrato input→output; A7 = "crear un arnés
  = definir sus fases"). Axiomas DAOP en crudo: `docs/architecture/boundaries/orquestacion-determinista-entre-cajas.md:25` (A2 = orquestación determinista), `METODOLOGIA.md:301` (A7 = document-as-cache),
...
- `VISION.md:225` "arch/ = **77 checks**" → real **97** (`docs/architecture/INDEX.md:67`). Faltan +12 de HS-06 y +8
...
- `docs/architecture/boundaries/permisos-derivan-del-rol.md:45` "Permiso efímero (spike **HS-07**)" — HS-07 fue
  doctrina-as-code, **no** hizo el spike; CLAUDE.md lo agenda para HS-08.
- `docs/architecture/contracts/api/openapi.yaml:176-197` `POST /sessions/{id}/permission` es **stub allow/deny HITL**:
...
- **m2 · Ficha huérfana del runner unificado:** `docs/architecture/knowledge/INDEX.md:86-88` y `docs/architecture/INDEX.md:22-25`
...
- **m9 · Ruido taxonómico en `harness-profile`:** `arquetipo-declarado` (`:128`) vive en el nodo de
  ...
  "L1.4 · **L2**" (L2 sin número, rompe el patrón de trazabilidad). `nota "77 previos"` (`docs/architecture/INDEX.md:76`)
