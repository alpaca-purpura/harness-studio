...
- **Fase B ✓ COMPLETA — 🛑 Gate 1 (mockup) FIRMADO por el operador (2026-07-06, commit `0736d2c`).**
  ...
  - **Las 6 decisiones son PROPUESTAS a cementar as-code (VISION/METODOLOGIA/docs/architecture/knowledge/arch) en Fase D.**
...
- **⚠️ Auditoría 5-subagentes (2026-07-06, build/test real) destapó INVERSIÓN DE ORDEN.** Se escribió
  **código adelantándose a los gates**: Fase F (Hito 1) YA implementada en el working-tree (loader del
  ...
  (VISION/METODOLOGIA/docs/architecture/knowledge/arch) **NO refleja** las 6 decisiones que el código ya materializó; (ii)
  🚩 `docs/architecture/boundaries/fe-visual-fitness.md` quedó flipado a `status: enforced` apuntando a un enforcer
...
- **Decisión del operador: RETRO-AJUSTAR EN ORDEN.** (1) ✓ **Gate 2 FIRMADO**. (2) **Fase D de verdad**:
  ...
  conteos (docs/architecture/knowledge/arch) + C4/contratos + graduar las PROPUESTAs (`origen`/`alw`) de fixture a L0
...
4. **Bajar as-code (Fase D)** las 6 decisiones firmadas (VISION/METODOLOGIA/docs/architecture/knowledge/arch): término «Base»,
...
4. As-code FE: `docs/architecture/boundaries/fe-*.md` (topologia-fsd · taxonomia-componentes **canvas⊥chrome** ·
   transporte-independiente · tokens-contrato · visual-fitness) y `docs/architecture/conventions/`.
...
- **No se programa sin arquitectura (NORMA):** antes de CUALQUIER código se revisa la arquitectura as-code
  (`docs/architecture/boundaries` + `docs/architecture/model` C4 + `docs/architecture/contracts` + mapeo `go-arch-lint`) y **se actualiza cuando el
...
- **As-code al día:** al mover un boundary de `proposed`→`enforced`, edita su `status:` y re-sincroniza los
  conteos (docs/architecture/knowledge/arch). No fabricar `pass`: un check sin enforcer determinista queda `deferred` honesto.
...
- **Boundaries FE del Mapa** (`docs/architecture/boundaries/fe-*.md`): confirmar/actualizar que `widgets/map-canvas` (canvas)
...
- **Boundaries backend de la deuda** (`docs/architecture/boundaries/*.md`): el adapter concreto de `ports.ArtifactReader` y
...
- **Modelo C4** (`docs/architecture/model/`): actualizar contexto/contenedor si nace un flujo nuevo (el Mapa como vista, el
  loader del índice, el endpoint de permisos).
- **Contratos** (`docs/architecture/contracts/`): si el Mapa necesita tipos generados (grafo TS) o `control_request` un shape
...
1. **Cablear `BoxConductor` + `KitProvisioner` al daemon.** Diseñar e implementar un adapter concreto de
   ...
   de permisos. Hoy ambos existen reales+testeados pero solo instanciados en `docs/architecture/fitness`.
...
- **G-hito1:** el arnés dogfood renderiza en el Mapa vivo (bandas + carriles), navegable, consola limpia,
  screenshots.
- **Cierre:** conteos as-code re-sincronizados (docs/architecture/knowledge/arch), ficha **HS-09** actualizada en LEDGER,
