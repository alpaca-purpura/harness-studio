...
*Cierre deep-link (2026-07-23):* investigación reveló que el TODO no era un bug — no había
NINGÚN código de deep-link (sin dep, sin scheme registrado, sin permiso). El BACKLOG lo
describía mal como «bug en callback». tauri#12726 confirmado real pero acotado a `tauri dev`
en Windows (build instalado no lo sufre). Alcance decidido con el operador: FE no tiene
router (`App.tsx` = 1 store Zustand, sin rutas URL) → el deep-link **solo enfoca la app**,
sin rutear a página (eso queda para paquete propio si se pide). Construido: `tauri-plugin-deep-link`
2.4.9 + `plugins.deep-link.desktop.schemes: ["arnesia"]` en `tauri.conf.json` (deb/rpm/AppImage
generan el registro OS solos) + permiso `deep-link:default` + `register_all()` en `setup()`
para Linux dev + reenfoque vía el mismo callback single-instance ya existente (CAP-79). Nueva
capability CAP-101 `tauri.deep-link-enfoca-app`. `cargo check` + `cargo clippy --no-deps`
verdes. **No verificado:** bundle real (`.deb`) con doble-click de un link `arnesia://` —
la rebuild de bundle queda para la próxima corrida de `make installer`.
