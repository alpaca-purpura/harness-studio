...
*Cierre deuda «loader detector 3°» (2026-07-23):* investigación confirmó que la deuda quedó STALE
sin actualizarse — el detector 3° SÍ se construyó, en Slice 0 «Cimientos» del Portafolio
(2026-07-13, ver `ledger/HS-23.md`), pero en `internal/adapters/portafolio/scanner.go#escanearLock`,
no en `internal/adapters/loader/` (el loader del Mapa se mantuvo deliberadamente single-arnés-
por-vista, `decisiones.md:35` de ese paquete: los adapters no se importan entre sí). `escanearLock`
parsea `.devstudio/arneses.yaml` (`arneses[]` con `id·version·canal·registry`, schema en
`docs/architecture/contracts/nomenclatura-arnes.md`), resuelve cada entrada contra
`~/.dev-studio/arneses/<id>/<version>/` y emite un `HallazgoInstalacion` por entrada — no
encontrada ⇒ aviso VISIBLE «declarada-en-lock, ausente», nunca silenciosa. Testeado
(`TestScannerLockDevstudio`, `scanner_test.go`). BACKLOG corregido (item borrado, era el mismo
trabajo con otro nombre).
