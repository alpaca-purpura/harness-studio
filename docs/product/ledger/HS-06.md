...
[`superficie-local-confinada`](./docs/architecture/boundaries/superficie-local-confinada.md) (middleware `withAuth`
en 3 gates: Host anti-rebinding · Origin allowlist reflejado —adiós CORS `*`— · token constant-time;
6 checks) + [`sesion-viva-consistente`](./docs/architecture/boundaries/sesion-viva-consistente.md) (un-turno-a-la-vez
...
el conductor. **Fitness tests reales que PASAN** (`go test ./docs/architecture/fitness/...`, `-race` limpio):
...
que esto ejecuta; arch FE) · `docs/architecture/knowledge/headless-sdk` (`--max-turns`, session-pin — dogfood) · I-76/
