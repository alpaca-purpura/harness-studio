...
[`superficie-local-confinada`](../../architecture/boundaries/superficie-local-confinada.md) (middleware `withAuth`
en 3 gates: Host anti-rebinding · Origin allowlist reflejado —adiós CORS `*`— · token constant-time;
6 checks) + [`sesion-viva-consistente`](../../architecture/boundaries/sesion-viva-consistente.md) (un-turno-a-la-vez
