...
2. **Tokens** — ¿en qué se van los tokens? Nodo: `carga X · 30d Y` con **fórmula única
   ...
   solapa** (docs/architecture/knowledge/reglas cuentan dentro de quien las carga — Σ ≠ total facturado).
   Fuente: usage JSONL.
...
4. **Proceso** — ¿fluye el proceso declarado? Franja de **flujo fase→fase** con volúmenes ·
   **saltos de fase como chips warn → abren SU corrida real** · tipología de excepciones
   ...
   excepciones detectadas) · docs/architecture/knowledge/reglas/mcp atenuados con «—» (no protagonizan
...
- **11 nodos `docs/architecture/knowledge/elements/*.md`** — skills, hooks, rules, subagents, commands, mcp,
...
docs/architecture/knowledge/reglas-raíz · 1 MCP. Verificado headless (jsdom + Playwright, 141 nodos render,
