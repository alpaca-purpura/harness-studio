...
| **13** | `capabilities-a-crear.md` CAP-139 y `arquitectura-modulo.md` §1 | CAP-139 apunta a `valor-o-sin-dato.tsx#ValorOSinDato`, `chip-confianza.tsx#ChipConfianza` y `widgets/mejora/ui/tarjeta-mejora.tsx#TarjetaMejora`, y §1 dibuja `web/src/widgets/mejora/` | **ninguno sobrevive** a `design.md` §1.3-1.4 / `plan-storybook.md` §1.1, que fijan `cifra-usd.tsx#CifraUSD`, `marca-confianza.tsx#MarcaConfianza` y `punto-mejora-card.tsx#PuntoMejoraCard` y **eliminan la slice `widgets/mejora/`** (chocaría con `no-sibling-widget-imports`, que está en `error`). **Gana `design.md`** — D22: CAP-139 se crea con los cuatro punteros nuevos y `widgets/mejora/` **no se crea**. Con los punteros viejos, `TestCapabilityPointerSymbolsResolve` falla |
