...
- [x] Fase 4 — cementado as-code: `docs/architecture/boundaries/superficie-local-confinada.md` v1.2→
      v1.3 (2 checks nuevos `mcp-config-siempre`/`setting-sources-siempre`, 7→9,
      `enforced_by:` real — `docs/architecture/fitness/hs17_config_source_test.go`, archivo NUEVO
      ...
      lo reconoce igual) · `docs/architecture/knowledge/elements/mcp.md` v1.0→v1.1 (nota, sin checks
...
- **Resultado:** `--mcp-config`+`--strict-mcp-config` (RF-161) + `--setting-sources
  ...
  `docs/architecture/boundaries/superficie-local-confinada.md` v1.3.
