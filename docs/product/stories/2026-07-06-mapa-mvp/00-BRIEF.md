...
- **Cero contradicción con doctrina** (VISION/METODOLOGIA/docs/architecture/knowledge/arch). Verifica con **subagentes**,
...
- **Modelo de datos L0** (claves español EXACTAS): `nodos`(id·clase·banda·fase?·caja?·alw?·prop?·trans?·
  ...
  Mapear ↔ `internal/domain/graph.go`+`box.go` ↔ `docs/architecture/contracts/schema/graph.l0.schema.json`. Marcar
...
1. **Estado actual — revisar A DETALLE nuestra arquitectura as-code:** boundaries FE
   (`docs/architecture/boundaries/fe-topologia-fsd`, `fe-taxonomia-componentes` **canvas⊥chrome**,
   `fe-transporte-independiente`, `fe-tokens-contrato`, `fe-visual-fitness`) + backend; modelo C4
   (`docs/architecture/model/`); contratos (`docs/architecture/contracts/` schemas L0 + box); mapeo `go-arch-lint`; convenciones
   (`docs/architecture/conventions/`). Confirmar qué existe / qué falta / qué está `proposed` vs `enforced`.
...
- **Backend:** `internal/domain/graph.go`+`box.go` · `internal/adapters/index/store.go` ·
  `dogfood/dev-full-cycle.graph.json` · `docs/architecture/contracts/schema/{graph.l0,box.contract}.schema.json`
- **Arch as-code:** `docs/architecture/boundaries/fe-*.md` · `docs/architecture/model/` · `docs/architecture/contracts/` · `docs/architecture/conventions/` ·
  `.go-arch-lint.yml`
- **Doctrina:** `VISION.md` (A6) · `METODOLOGIA.md` (§3 contrato, §8.3 document-as-cache) · `UX.md` (S2/S3)
  · `docs/architecture/knowledge/` (nodes)
