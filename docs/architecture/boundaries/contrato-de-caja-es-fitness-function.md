...
- **Puente con `docs/architecture/knowledge/`:** este check es la versión-arquitectura del estándar de `docs/architecture/knowledge/`
  (138 checks). El motor **`arnesia conformance`** (construido en HS-08) parsea `docs/architecture/knowledge/` +
...
- 2026-07-07 · v1.2 · Sync HS-10: la referencia «linter de 122 checks» apuntaba a un conteo y a un
  runner futuros — el motor real es `arnesia conformance` (construido en HS-08; `docs/architecture/knowledge/` = 138
  checks, `docs/architecture/knowledge/`+`arch/` = 235 checks a datos). Sin cambios de checks ni de status.
...
- 2026-07-08 · v1.4 · Auditoría colateral (HS-16): `sin-huerfanos`/`dead-end`/`ruta-a-existe`/
  ...
  `TestDogfoodComposicionFabricaConforma` (`docs/architecture/fitness/arch_test.go`) reusa el mismo
