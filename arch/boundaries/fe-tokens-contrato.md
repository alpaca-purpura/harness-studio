...
- **SSOT = `web/tokens/*.tokens.json` (DTCG, dual-theme).** Verificado: los valores extraídos calzan
  ...
  de componente (`--c-skill/agent/hook/know/mcp/rule`, mapea a `knowledge/`), salud de trazas
...
| no-magic-value | ningún `color/background/border/fill/shadow` en CSS a mano usa hex/valor crudo, solo `var(--…)` | error | banda Base «valor mágico (rompe el contrato de tokens)» | stylelint#strict-value |
