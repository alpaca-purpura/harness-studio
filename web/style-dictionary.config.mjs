...
function cssVar(path) {
  ...
  if (g0 === "color") {
    ...
    if (g1 === "sidebar")
      return rest[0] === "background" ? "--sidebar" : `--sidebar-${rest.join("-")}`;
  }
  return `--${path.join("-")}`;
}
...
function resolve(value, byPath) {
  if (typeof value !== "string") return Array.isArray(value) ? value.join(", ") : value;
  const m = value.match(/^\{(.+)\}$/);
  if (!m) return value;
...
StyleDictionary.registerFormat({
  ...
  format: ({ dictionary }) => {
    const byPath = new Map(dictionary.allTokens.map((t) => [t.path.join("."), t.original]));
    const light = [];
    const dark = [];
