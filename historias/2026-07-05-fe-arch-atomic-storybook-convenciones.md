...
> [`../arch/`](../arch/INDEX.md) (gemelo FE de los 7 boundaries backend de HS-04). Condensado:
...
2. **Un solo enforcer de grafo, no dos.** A recomienda `dependency-cruiser` como CI-gate y
   ...
   **Resuelto: dependency-cruiser = gate** (declarativo standalone, espeja go-arch-lint, emite
   DOT/SVG para `arch/model/`); `steiger` = check FSD-estructural secundario (beta, no único gate);
...
**Disposición as-code (propuesta):** **`arch/conventions/`** como hermano de `arch/fitness/` —
...
`knowledge/` + `arch/` + `arch/conventions/` (3 árboles). `arch/conventions/CADENCE.md` = «se revisa al
...
componente** (mapea a `knowledge/`): `--c-skill #2f74b5 · --c-agent #7455c2 · --c-hook #bb4c85 ·
